...
def game_id_gen(header):
    if 'Link' in header:
        return header['Link'].split('/')[-1]
    elif header['Site'] == "Chess.com":
...
def gameToDF(input_game, per_game_vals = None, per_move_vals = None, with_board_stats = True, allow_non_sf = False):
    """Hack to make dataframes instead of CSVs while maintaining the same code as much as possible"""
...
def gameToCSVlines(input_game, per_game_vals = None, per_move_vals = None, with_board_stats = True, allow_non_sf = True):
    ...
    for n in per_game_vals:
        try:
            ...
            elif n in ['time_control', 'termination', 'white_elo', 'black_elo', 'start_time']:
                gameVals.append('')
            else:
                raise KeyError(f"{n} {e} for header: {game.headers}\ngame: {input_game}")

    gameVals = [str(v).replace(',', '') for v in gameVals]

    white_won = game.headers['Result'] == '1-0'
    no_winner = game.headers['Result'] not in  ['1-0', '0-1']

    time_per_player = time_control_to_secs(game.headers.get('TimeControl', '999999+0'))
    ...
    white_player = game.headers['White'].replace(',', '')
    black_player =  game.headers['Black'].replace(',', '')
    ...
    for i, node in enumerate(game.mainline()):
        ...
        act_elo = game.headers.get('WhiteElo', '9999') if is_white else game.headers.get('BlackElo', '9999')
        opp_elo = game.headers.get('BlackElo', '9999') if is_white else game.headers.get('WhiteElo', '9999')
        if no_winner:
            act_won = False
        elif is_white:
            act_won = white_won
