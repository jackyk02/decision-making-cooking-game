...
class Embed_LeelaBoard:
    ...
    def lcz_features(self):
        ...
        curdata = self.lcz_stack[-1]
        planes_yielded = 0
        # print(self.lcz_stack[-1:-9:-1])
        # print("")
        # for data in self.lcz_stack[-1:-9:-1]:
        for data in self.lcz_stack[-1:-3:-1]:
            plane_bytes = data.plane_bytes
            if not curdata.side_to_move:
                # we're white
                planes = (np.unpackbits(memoryview(plane_bytes))[::-1]
                                        .reshape(12, 8, 8)[::-1])
                # planes = np.flip(planes, 1)
        ...
        empty_planes = [flat_planes[0] for _ in range(26-planes_yielded)]
