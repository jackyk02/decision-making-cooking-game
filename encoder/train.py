...
from encoder.data_objects import PlayerVerificationDataLoader, PlayerVerificationDataset
...
def train(run_id: str, data_dir:str, validate_data_dir:str, models_dir: Path, umap_every: int, 
    ...
    # Training loop
    profiler = Profiler(summarize_every=100, disabled=True)
    for step, player_batch in enumerate(train_loader, init_step):
        ...
        if umap_every != 0 and step % umap_every == 0:
            ...
            vis.draw_projections(embeds, games_per_player, step, projection_fpath)
            vis.save()
        ...
        if backup_every != 0 and step % backup_every == 0:
            if step > 5000: # don't save until 5k steps
        ...
        if validate_every != 0 and step % validate_every == 0:
            ...
            vis.draw_projections(validate_embeds, v_games_per_player, step, projection_fpath, is_validate=True)
            vis.save()
