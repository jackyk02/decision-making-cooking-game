...
class Visualizations:
    ...
    def log_dataset(self, dataset: PlayerVerificationDataset):
        if self.disabled:
            return 
        dataset_string = ""
        dataset_string += "<b>Players</b>: %s\n" % len(dataset.players)
        dataset_string += "\n" + dataset.get_logs()
    ...
    def draw_projections(self, embeds, games_per_player, step, out_fpath=None,
                         max_players=20, is_validate=False):
        max_players = min(max_players, len(colormap))
        embeds = embeds[:max_players * games_per_player]
        
        n_players = len(embeds) // games_per_player
        ground_truth = np.repeat(np.arange(n_players), games_per_player)
